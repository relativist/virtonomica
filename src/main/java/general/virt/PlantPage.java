    ...
    public PlantPage supply(){
        ...
        for(int i =0; i<driver.findElements(By.xpath("//tr[contains(@id,'product_row')]//a[@title]")).size();i++){
            ...
            if(driver.findElements(By.xpath("//tr[contains(@id,'product_row')]["+(i+1)+"]/td")).size()>6){
                error = "";
                title = driver.findElement(By.xpath("//tr[contains(@id,'product_row')]["+(i+1)+"]//a[@title]")).getAttribute("title");
                need = driver.findElement(By.xpath("//tr[td[contains(text(),'Требуется')]]["+(i+1)+"]/td[2]")).getText().replaceAll(" ", "");
                have = driver.findElement(By.xpath("//tr[td[contains(text(),'Количество')]]["+(i+1)+"]/td[2]")).getText().replaceAll(" ","");
                offer = driver.findElement(By.xpath("//tr[contains(@id,'product_row')]/td[4]["+(i+1)+"]//input")).getAttribute("value").replaceAll(" ", "");
                sklad = driver.findElement(By.xpath("//tr[contains(@id,'product_row')]/td[7]["+(i+1)+"]//tr[2]/td[2]")).getText().replaceAll(" ", "");

                if(Integer.valueOf(have)>2*Integer.valueOf(need)){
                    if(!offer.equals("0")){
                        driver.findElement(By.xpath("//tr[contains(@id,'product_row')]/td[4]["+(i+1)+"]//input")).clear();
                        driver.findElement(By.xpath("//tr[contains(@id,'product_row')]/td[4]["+(i+1)+"]//input")).sendKeys("0");
                    }
                    continue;
                }
                ...
                if(Integer.valueOf(have)<2*Integer.valueOf(need)){
                    driver.findElement(By.xpath("//tr[contains(@id,'product_row')]/td[4]["+(i+1)+"]//input")).clear();
                    driver.findElement(By.xpath("//tr[contains(@id,'product_row')]/td[4]["+(i+1)+"]//input")).sendKeys(need);
                    change=true;
                }
