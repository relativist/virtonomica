...
public class RepairPage extends Page {
    ...
    public int repairIt() throws InterruptedException {
        ...
        else if (driver.findElement(By.xpath("//div[@class='officePlace']")).getText().split(" ")[0].equals("Лаборатория")){
            DepartmentType="Science";
            logMe(DepartmentType);
            EQ=Double.valueOf(driver.findElement(By.xpath("//tr[td[text()='Качество оборудования']]/td[2]")).getText().split(" ")[0]);
            eqNeed=EQ;
            toRepair=Integer.valueOf(driver.findElement(By.xpath("//tr[td[text()='Износ оборудования']]/td[2]//td[2]")).getText().split(" % ")[1].split("\\+")[0].replaceAll("\\D+",""))+1;
            if(toRepair<10){
                logMe("No need repair! ");
