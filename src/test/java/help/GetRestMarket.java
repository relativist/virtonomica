...
public class GetRestMarket extends HelpPage {
...
//    }
    ...
    public void test() throws Throwable {
        ...
        driver.get("http://virtonomica.ru/vera/main/globalreport/marketing/by_service/373265");
...
//        Больница
        ...
        File file = new File("rest.db");
        if(!file.exists()) {
            logMe("creating new database table!");
            new CreateDB().createRestDb();
        }
        ...
        for(int i=1; i<s1.getOptions().size(); i++){
            ...
            for(int j=1; j<s2.getOptions().size(); j++){
                ...
                for(int k=1; k<s3.getOptions().size(); k++){
                    ...
                    if(driver.findElements(By.xpath("//table/tbody/tr[td[text()='Кофейня']]/td[5]")).size()==0)
                        coffee="0";
                    else
                        coffee = driver.findElement(By.xpath("//table/tbody/tr[td[text()='Кофейня']]/td[5]")).getText().split(" ")[0];

                    if(driver.findElements(By.xpath("//table/tbody/tr[td[text()='Ресторан греческой кухни']]/td[5]")).size()==0)
                        greece="0";
                    else
                        greece = driver.findElement(By.xpath("//table/tbody/tr[td[text()='Ресторан греческой кухни']]/td[5]")).getText().split(" ")[0];

                    if(driver.findElements(By.xpath("//table/tbody/tr[td[text()='Устричный ресторан']]/td[5]")).size()==0)
                        ustr="0";
                    else
                        ustr = driver.findElement(By.xpath("//table/tbody/tr[td[text()='Устричный ресторан']]/td[5]")).getText().split(" ")[0];
...
//                    logMe(cnm);
                    ...
                    recordRest(city,coffee,greece,ustr,price,volume,count);
