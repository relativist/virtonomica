...
public class GetMedicalMarket extends HelpPage {
...
//    }
    ...
    public void test() throws Throwable {
        ...
        driver.get("http://virtonomica.ru/vera/main/globalreport/marketing/by_service/359926");
...
//        Больница
        ...
        File file = new File("medical.db");
        if(!file.exists()) {
            logMe("creating new database table!");
            new CreateDB().createMedicalDb();
        }
        ...
        for(int i=1; i<s1.getOptions().size(); i++){
            ...
            for(int j=1; j<s2.getOptions().size(); j++){
                ...
                for(int k=1; k<s3.getOptions().size(); k++){
                    ...
                    if(driver.findElements(By.xpath("//table/tbody/tr[td[text()='Больница']]/td[5]")).size()==0)
                        bol="0";
                    else
                        bol = driver.findElement(By.xpath("//table/tbody/tr[td[text()='Больница']]/td[5]")).getText().split(" ")[0];

                    if(driver.findElements(By.xpath("//table/tbody/tr[td[text()='Диагностический центр']]/td[5]")).size()==0)
                        dc="0";
                    else
                        dc = driver.findElement(By.xpath("//table/tbody/tr[td[text()='Диагностический центр']]/td[5]")).getText().split(" ")[0];

                    if(driver.findElements(By.xpath("//table/tbody/tr[td[text()='Поликлиника']]/td[5]")).size()==0)
                        pol="0";
                    else
                        pol = driver.findElement(By.xpath("//table/tbody/tr[td[text()='Поликлиника']]/td[5]")).getText().split(" ")[0];

                    if(driver.findElements(By.xpath("//table/tbody/tr[td[text()='Стоматологическая клиника']]/td[5]")).size()==0)
                        stomk="0";
                    else
                        stomk = driver.findElement(By.xpath("//table/tbody/tr[td[text()='Стоматологическая клиника']]/td[5]")).getText().split(" ")[0];

                    if(driver.findElements(By.xpath("//table/tbody/tr[td[text()='Центр народной медицины']]/td[5]")).size()==0)
                        cnm="0";
                    else
                        cnm = driver.findElement(By.xpath("//table/tbody/tr[td[text()='Центр народной медицины']]/td[5]")).getText().split(" ")[0];
...
//                    logMe(cnm);
                    ...
                    recordMedical(city,dc,pol,stomk,cnm,bol,price,volume,count);
