...
public class GetLaundryMarket extends HelpPage {
...
//    }
    ...
    public void test() throws Throwable {
        ...
        driver.get("http://virtonomica.ru/vera/main/globalreport/marketing/by_service/373255");
...
//        Больница
        ...
        File file = new File("laundry.db");
        if(!file.exists()) {
            logMe("creating new database table!");
            new CreateDB().createLaundry();
        }
        ...
        for(int i=1; i<s1.getOptions().size(); i++){
            ...
            for(int j=1; j<s2.getOptions().size(); j++){
                ...
                for(int k=1; k<s3.getOptions().size(); k++){
                    ...
                    if(driver.findElements(By.xpath("//table/tbody/tr[td[text()='Химчистка']]/td[5]")).size()==0)
                        him="0";
                    else
                        him = driver.findElement(By.xpath("//table/tbody/tr[td[text()='Химчистка']]/td[5]")).getText().split(" ")[0];

                    if(driver.findElements(By.xpath("//table/tbody/tr[td[text()='Прачечная']]/td[5]")).size()==0)
                        pr="0";
                    else
                        pr = driver.findElement(By.xpath("//table/tbody/tr[td[text()='Прачечная']]/td[5]")).getText().split(" ")[0];

                    if(driver.findElements(By.xpath("//table/tbody/tr[td[text()='Прачечная самообслуживания']]/td[5]")).size()==0)
                        prsam="0";
                    else
                        prsam = driver.findElement(By.xpath("//table/tbody/tr[td[text()='Прачечная самообслуживания']]/td[5]")).getText().split(" ")[0];
...
//                    logMe(cnm);
                    ...
                    recordLaundry(city,prsam,him,pr,price,volume,count);
